    ...
    def songGen(self):
        ...
        def getResponseChatgpt(u_model):
            response = client.chat.completions.create(
                ...
                messages=[
                    {"role": "assistant",
                     "content": "This is the settings taken from a certain photo. Look at this and recommend a song in your own way. You must not say anything other than the YouTube link. Like: https://www.youtube.com/watch?v=VdQY7BusJNU"},
                    {"role": "user",
                     "content": f"camera brand:{data[0]},camera model:{data[1]},The time taken:{data[2]},Shutter Speed:{data[3]},ExposureTime:{data[4]},ISO:{data[5]},Fnumber:{data[6]}Lens:{data[7]}"},
                ]
            )
            return response
